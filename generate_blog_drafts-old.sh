...
mkdir -p blog_post_drafts
total_files=$(find blog_post_ideas -maxdepth 1 -type f -name "*.md" | wc -l)
current=0

for idea_file in blog_post_ideas/*.md; do
    current=$((current + 1))
    echo "[$current/$total_files] Processing $(basename "$idea_file")"
    idea_content=$(cat "$idea_file")
    context_content=$(cat context.md)
    prompt="Using Construkted Reality's content strategy (from context.md), create a Conceptual & Vision Content blog post for the following idea: $idea_content. Focus on validating the user pain point while mentioning our product directly. load the content from the SOURCE URLs provided and use as further context. Context content for what Construkted Reality does: $context_content"
    ...
    # Save the response
    echo "$response" > "blog_post_drafts/$(basename "$idea_file")"
done
