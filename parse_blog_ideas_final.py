#!/usr/bin/env python3
"""
Final script to parse blog post idea files and standardize their formatting.
This script processes each file correctly by splitting on blog post headers.
"""
...
def split_by_headers(content: str, is_photogrammetry: bool) -> List[str]:
    """
    Split content by blog post headers.
    
    Args:
        content (str): File content
        is_photogrammetry (bool): Whether this is a photogrammetry file
        
    Returns:
        List[str]: List of individual blog post sections
    """
    if is_photogrammetry:
        # Pattern for photogrammetry files: ## 1. Title
        header_pattern = r'^##\s*(\d+\.\s+.+)'
    else:
        # Pattern for mobile LiDAR files: ## Blog Post Idea X: "Title"
        header_pattern = r'^##\s*Blog Post Idea\s+\d+:\s*"(.+)"'
    
    # Split content by headers, keeping the headers
    parts = re.split(header_pattern, content, flags=re.MULTILINE)
    
    # Reconstruct sections with headers
    sections = []
    for i in range(1, len(parts), 2):  # Skip empty first part
        if i + 1 < len(parts):
...
def parse_mobile_lidar_file(file_path: str) -> List[Dict]:
    ...
    # Split by headers
    sections = split_by_headers(content, False)
...
def parse_photogrammetry_file(file_path: str) -> List[Dict]:
    ...
    # Split by headers
    sections = split_by_headers(content, True)
