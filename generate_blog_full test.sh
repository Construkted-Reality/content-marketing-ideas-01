...
for idea_file in blog_post_drafts/*.md; do
    ...
    prompt="Using Construkted Reality's content marketing strategy, create a full blog post using the following draft: $idea_content. Focus on validating the user pain point hand take every opportunity to mentioning our product Construkted Reality. fir images, suggest prompts I can feed an llm for image generation throughout the post. Do not use tables unless absolutely necessary. "

    # Prepare the JSON payload
    JSON_PAYLOAD=$(jq -n \
        --arg model "$DEFAULT_MODEL" \
        --arg prompt "$prompt" \
        --arg system_prompt "Reasoning: low,  extra context $context_content" \
